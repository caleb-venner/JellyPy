...
# Jellypy Plugin Beta Release Script
# This script automates the release process for beta versions (x.x.x.1-999)
# For stable releases, use release.sh
...
update_csproj_version() {
    ...
    # Extract 3-digit version (remove last digit for Version tag)
    local short_version=$(echo "$new_version" | sed 's/\.[0-9]*$//')
...
update_manifest_checksum() {
    local version=$1
    local checksum=$2
    local manifest_file="manifest-beta.json"
...
show_release_summary() {
    ...
    echo ""
    echo "=========================================="
    print_success "BETA RELEASE SUMMARY"
    echo "=========================================="
    ...
    echo "2. Commit the changes: git add . && git commit -m 'Release v$version (beta)'"
    echo "3. Push to GitHub: git push origin main"
    echo "4. Plugin will be available immediately via beta repository URL"
...
# Main function
main() {
    print_status "Starting Jellypy BETA release process..."
    ...
    if [ $# -eq 1 ]; then
        ...
        if [[ $update_version =~ ^[Yy]$ ]]; then
            echo -n "Enter new beta version (x.x.x.y format, where y > 0): "
            read -r new_version
            
            # Validate version format
            if [[ ! $new_version =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
                print_error "Invalid version format. Use x.x.x.y format (e.g., 1.1.2.1)"
                exit 1
            fi
    ...
    if [[ ! $version =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        print_error "Invalid version format: $version. Use x.x.x.y format (e.g., 1.1.2.1)"
        exit 1
    fi
    
    # Validate this is a beta version (must NOT end in .0)
    if [[ $version =~ \.0$ ]]; then
        print_error "This script is for beta releases only (last digit must be > 0, e.g., 1.1.2.1)"
        print_error "For stable releases (x.x.x.0), use release.sh instead"
        exit 1
    fi
    
    print_status "Creating BETA release for version $version"
    ...
    print_success "Beta release process completed successfully!"
}
