...
/// <summary>
/// Handler for playback pause events.
/// </summary>
public class PlaybackPauseHandler : IEventProcessor<PlaybackProgressEventArgs>
{
    private static readonly EventDeduplicationCache DeduplicationCache = new(TimeSpan.FromSeconds(5));

    private readonly ILogger<PlaybackPauseHandler> _logger;
    private readonly IScriptExecutionService _scriptExecutionService;

    /// <summary>
    /// Initializes a new instance of the <see cref="PlaybackPauseHandler"/> class.
    /// </summary>
    /// <param name="logger">The logger.</param>
    /// <param name="scriptExecutionService">The script execution service.</param>
    public PlaybackPauseHandler(
        ILogger<PlaybackPauseHandler> logger,
    ...
    /// <inheritdoc />
    public EventType EventType => EventType.PlaybackPause;
    ...
    public EventData ExtractEventData(PlaybackProgressEventArgs eventArgs)
    {
        ...
        {
            EventType = EventType.PlaybackPause,
    ...
    public async Task HandleAsync(PlaybackProgressEventArgs eventArgs)
    {
        try
        {
            ...
            // Deduplicate rapid pause events for the same session to avoid script thrashing
        ...
        {
            _logger.LogError(ex, "Error handling PlaybackPause event for item {ItemName}", eventArgs.Item?.Name);
